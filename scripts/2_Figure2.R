...
Fibs.integrated <- SetIdent(Fibs.integrated, value = "Fibs_MajorClusters")
Fibs.integrated_samples <- AverageExpression(Fibs.integrated,
...
Fibs.integrated_samples@meta.data <-
  Fibs.integrated_samples@meta.data %>%
  mutate(
    Fibs_MajorClusters = str_split_fixed(colnames(Fibs.integrated_samples), "_", 3)[, 1],
...
keepCols_metaData <- c(
  "SampleID", "nCount_RNA", "nFeature_RNA",
...
Sample.level_Markers <- FindAllMarkers(Fibs.integrated_samples,
...
Marker.match <- match(
  paste(All.Markers$cluster, All.Markers$gene),
...
Fig_2C <-
  DoHeatmap(Fibs.integrated_samples,
    features = rev(top10_markers$gene),
    assay = "RNA",
...
REACTOME_gsva_res <- gsva(as.matrix(Fibs.integrated_samples@assays$RNA@data),
  gset.idx.list = Reactome_pathwaysH, method = "gsva",
...
contrast.matrix <- makeContrasts(
  Myo.v.ALL = Myo - (Alveolar + Adventitial) / 2,
...
Fig_2D <-
  ...
  ggplot(aes(
    x = reorder(Pathway.label, logFC), y = logFC, label = paste0("adj.P=", signif(adj.P.Val, 3)),
    fill = Cluster
  )) +
  theme_pubr(base_size = 5) +
  theme(
...
All.Markers$type <- factor(
  ...
  levels = 1:5,
  labels = c(
    "Basement.Membranes", "Interstitial.Collagens",
...
pA <-
  Fibs.integrated_samples@meta.data %>%
  ggplot(aes(
    x = Fibs_MajorClusters, y = Basement.Membranes,
  ...
  stat_compare_means(
    comparisons = list(c("Myo", "Adventitial"), c("Alveolar", "Adventitial")),
    # label = "p.signif",
    size = 2.5,
...
pB <-
  Fibs.integrated_samples@meta.data %>%
  ggplot(aes(
    x = Fibs_MajorClusters, y = Interstitial.Collagens,
    fill = Fibs_MajorClusters
  )) +
  theme_pubr(base_size = 5) +
  theme(legend.position = "right", axis.title.x = element_blank()) +
  ...
  geom_jitter(width = 0.2, alpha = 0.5, size = 1) +
  stat_compare_means(
    comparisons = list(c("Myo", "Adventitial"), c("Myo", "Alveolar")),
    # label = "p.signif",
    size = 2.5,
...
Batchtest_raw <- Batch.Quant(
  seurat_obj = Fibs.integrated,
  vars = c("Dataset"),
  nPerm = 1000, assay_nn = "RNA"
)

Batchtest_cca <- Batch.Quant(
  seurat_obj = Fibs.integrated,
  vars = c("Dataset"),
...
for (i in c(
  ...
  Fibs.integrated_samples.test <- AverageExpression(Fibs.integrated,
    assays = "RNA",
    return.seurat = T,
  ...
  Strict.Markers.samples[[i]] <- FindAllMarkers(Fibs.integrated_samples.test,
    test.use = "wilcox", assay = "RNA",
    logfc.threshold = 1,
    min.pct = 0.5,
    only.pos = T
...
SupplFig_2C <-
  ...
  geom_point(
    data = Fibs.integrated@meta.data[, c("UMAP_1", "UMAP_2")],
...
for (i in names(Fibs.Subpop.list)) {
  ...
  TvN_markers_list[[i]] <- FindMarkers(
    Fibs.Subpop.list[[i]],
    ident.1 = "Tumour", ident.2 = "Control", assay = "RNA", only.pos = F
  )
